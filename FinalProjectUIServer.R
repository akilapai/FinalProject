...
ui <- shinyUI(fluidPage(
  ...
  navbarPage("Final Project",
             ...
             tabPanel("Aislyn DiRisio", 
                      fluidRow(titlePanel("Protein-Specific Expression by Treatment and Time"),
                               sidebarLayout(position = "right", sidebarPanel(selectInput("ProteinName", "Protein", choices=unique(FinalData$ProteinName), selected = "TNFB")),
                                             mainPanel(plotOutput("aislyn_plot"))))),
...
## JANE'S DATA
lessthan40 <- subset(FinalData, subset=Age<= 40)
over40 <- subset(FinalData, subset=Age>40)
IL8_1 <- subset(lessthan40, subset = ProteinName=="IL-8")
IL8_2 <- subset(over40, subset = ProteinName=="IL-8")
Expression_1 <- IL8_1$Expression
Expression_2 <- IL8_2$Expression
...
server <-shinyServer(function(input, output) {
  ...
  ###End Aislyn's Section
  
  
  ### START JANE'S SECTION###
  output$jane <- renderPlot({
    if(input$Age == "Less than or Equal to 40"){hist(Expression_1, main="Expression of IL-8 in People less than or equal to 40", xlab= "Expression", ylab = "Frequency",  col="lightblue",xlim = c(4,16))}
    
    if(input$Age == "Over 40"){hist(Expression_2, main="Expression of IL-8 in People over 40", xlab = "Expression", ylab = "Frequency",col="violet",xlim = c(4,16))}
  })
